...
export async function POST(request: Request) {
  try {
    ...
    const selections = [kills, deaths, cs, assists].filter(
    ...
    if (liveGameError || !liveGame) {
      return Response.json(
        { error: "Player not currently in a live game" },
        { status: 400 }
      );
    }
    ...
    if (!isUpdate) {
      ...
      if (amountDifference > balanceData.account_balance) {
        return Response.json(
          { error: "Not enough funds for the increased bet amount" },
          { status: 400 }
        );
      }
    }
    ...
    if (!isUpdate) {
      ...
      const { data: updateData, error: updateError } = await supabase
        .from("users")
        .update({
          account_balance: balanceData.account_balance - amountDifference,
        })
        .eq("user_id", user_id)
        .select()
        .single();
